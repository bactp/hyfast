...
PREFIX = "central-cluster_kube-system_pod_data"
SAVED_BUCKET = "central-cluster-training"
SAVED_BUCKET_PREFIX = "central-cluster_kube-system_pod_data_training"
...
def process_data(minio_client, object_name):
    ...
    """
    COLS = ['cluster_ns_pod_status_scheduled',
            'cluster_ns_pod_status_unscheduled',
            'cluster_ns_pod_status_unknown',
            'cluster_ns_pod_status_ready_true',
            'cluster_ns_pod_status_ready_false',
            'cluster_ns_pod_status_phase_pending',
            'cluster_ns_pod_status_phase_failed',
            'cluster_ns_pod_status_phase_running',
            'cluster_ns_pod_status_phase_succeeded',
            'cluster_ns_pod_status_phase_unknown',
            'cluster_ns_pod_status_reason_evicted',
            'cluster_ns_pod_status_reason_nodeaffinity',
            'cluster_ns_pod_status_reason_nodelost',
            'cluster_ns_pod_status_reason_shutdown',
            'cluster_ns_pod_status_reason_unexpectedadmissionerror']
    ...
    scaler = joblib.load("./central_cluster_ns_pod_scaler.joblib")

    scaled_df = pd.DataFrame(scaler.transform(df.drop(['pod', 'timestamp'], axis=1)), columns=COLS)
    scaled_df['pod'] = df['pod']
